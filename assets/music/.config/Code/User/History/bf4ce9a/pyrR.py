...
class PlayerEntryScreen(QWidget):
    ...
    def add_player_by_key(self):
        ...
        arrow_label, num_label, player_id_field, code_name_field, equip_id = row
    ...
    def toggle_visibility(self):        
            ...
            for index, (arrow_label, num_label, input1, input2, input3) in enumerate(combined_rows):
    ...
    def sort_players(self):
            for i in range(len(self.red_row) - 1):
                arrow_label, num_label, player_id_field, code_name_field,equip_id = self.red_row[i]

                if player_id_field.text().strip() == "" and code_name_field.text().strip() == "":
                    for j in range(i + 1, len(self.red_row)):
                        next_arrow, next_num, next_player_id, next_code_name, next_eqip_id = self.red_row[j]
            ...
            for i in range(len(self.green_row) - 1):
                arrow_label, num_label, player_id_field, code_name_field, equip_id  = self.green_row[i]

                if player_id_field.text().strip() == "" and code_name_field.text().strip() == "":
                    for j in range(i + 1, len(self.green_row)):
                        next_arrow, next_num, next_player_id, next_code_name, next_eqip_id = self.green_row[j]
