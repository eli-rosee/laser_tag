...
class PlayerEntryScreen(QWidget):
    ...
    def __init__(self):
        ...
        self.buttons[2].clicked.connect(self.change_ip)
    ...
    def ip_check(self, ip, popup):
        try:
            ipaddress.ip_address(ip)
            return True
        except ValueError:
            print("Error, incorrect IP address entered. Try again.")
            return False


    # Handles the change IP button functionality
    def construct_ip_popup(self):
        ...
        confirm_button.clicked.connect(lambda: self.ip_check(ip_input, popup))
